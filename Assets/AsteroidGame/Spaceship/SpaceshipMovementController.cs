...
public class SpaceshipMovementController : NetworkBehaviour
{
    ...
    private Rigidbody _rigidbody = null; // The Unity Rigidbody (RB) is automatically synchronised across the network thanks to the NetworkRigidbody (NRB) component.
    ...
    {
        Quaternion rot = _rigidbody.rotation * Quaternion.Euler(0, input.HorizontalInput * _rotationSpeed * Runner.DeltaTime, 0);
    ...
    private void CheckExitScreen()
    {
        ...
        position -= position.normalized * 0.1f; // offset a little bit to avoid looping back & forth between the 2 edges 
