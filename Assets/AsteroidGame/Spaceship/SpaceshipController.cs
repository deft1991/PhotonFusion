...
public class SpaceshipController : NetworkBehaviour
{
    ...
    private bool HasHitAsteroid()
    {
        ...
        var count = Runner.LagCompensation.OverlapSphere(_rigidbody.position, _spaceshipDamageRadius, Object.InputAuthority, _lagCompensatedHits,
            _asteroidCollisionLayer.value);

        if (count <= 0) return false;

        Sort(_lagCompensatedHits, count);
        _lagCompensatedHits[0].GameObject.GetComponent<AsteroidBehaviour>().HitAsteroid(PlayerRef.None);
    ...
    private void ShipWasHit()
    {
        ...
        // If testing the game with only a host player, the game will end after the first hit.
    ...
    // Utility function to sort hits by distance.
